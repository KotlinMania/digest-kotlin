...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `DigestKotlin.swiftmodule/`
...
//      `import DigestKotlin` above would fail at compile time.
//
//   2. The static archive `libDigestKotlin.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "DigestKotlin" }` and
//      `flattenPackage = "io.github.kotlinmania.digest"` configuration in
...
//      via the `DigestKotlinLibrary` product.
//
...
final class DigestExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "DigestKotlin swift module imported cleanly")
    }
}
